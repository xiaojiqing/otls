...
class OnlinePADOGen: public PADOParty<IO> { public:
	OnlineHalfGateGen<IO> * gc;
	PRG prg;
	OnlinePADOGen(IO* io, OnlineHalfGateGen<IO>* gc, IKNP<IO>* in_ot = nullptr): PADOParty<IO>(io, ALICE, in_ot) {
		this->gc = gc;
		if(in_ot == nullptr) {
			bool delta_bool[128];
