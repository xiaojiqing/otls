...
const string circuit_file_location = macro_xstr(EMP_CIRCUIT_PATH) + string("bristol_fashion/");
...
class AES_GCM {
   public:
    ...
    inline void enc(NetIO* io, unsigned char* ctxt, unsigned char* tag, unsigned char* iv, unsigned char* msg, size_t m, unsigned char* aInfo, size_t info_len, int party) {
        if (sizeof(iv) != 12) {
        ...
        if (party == ALICE) {
            size_t v = 128 * ((info_len * 8 + 128 - 1) / 128) - info_len * 8; // 128 * ceil(8*info_len/128) - info_len*8

            io->recv_data(ctxt, m);
            unsigned char* x = new unsigned char[u / 8 + m + v / 8 + info_len + 16];
            memcpy(x, aInfo, info_len);
            memset(x + info_len, 0, v / 8);
            memcpy(x + info_len + v / 8, ctxt, m);
            memset(x + info_len + v / 8 + m, 0, u / 8);
            memcpy(x + info_len + v / 8 + m + u / 8, (unsigned char*)&info_len, 8);
            memcpy(x + info_len + v / 8 + m + u / 8 + 8, (unsigned char*)&m, 8);

            block* xblk = (block*)&x;
            block t = ghash(h_z0[0], xblk, (u + 8 * m + v + 8 * info_len) / 128 + 1);
            t = t ^ h_z0[1];
            tag = (unsigned char*)&t;
            io->send_data(tag, 16);
            delete[] x;
