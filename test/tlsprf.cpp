...
void tlsprf_test() {
    vector<unsigned char> secret_u = {0x9b, 0xbe, 0x43, 0x6b, 0xa9, 0x40,
                                      0xf0, 0x17, 0xb1, 0x76, 0x52, 0x84,
                                      0x9a, 0x71, 0xdb, 0x35};
    vector<unsigned char> seed_u = {0xa0, 0xba, 0x9f, 0x93, 0x6c, 0xda,
                                    0x31, 0x18, 0x27, 0xa6, 0xf7, 0x96,
                                    0xff, 0xd5, 0x19, 0x8c};
...
void opt_tlsprf_test() {
    ...
    HMAC_SHA_256 hmac;
    tlsprf.init(hmac, secret);
    tlsprf.opt_prf(hmac, res, 800, secret, label, label_u.size(), seed,
                   seed_u.size(), true, true);
...
void opt_tlsprf_circ_test() {
    vector<unsigned char> secret_u = {
      ...
      0x84, 0x9a, 0x71, 0xdb, 0x35, 0x9b, 0xbe, 0x43, 0x6b, 0xa9, 0x40,
      0xf0, 0x17, 0xb1, 0x76, 0x52, 0x84, 0x9a, 0x71, 0xdb, 0x35};
    ...
    tlsprf.init(hmac, secret);
    tlsprf.opt_prf(hmac, res, 48 * 8, secret, label, label_len, seed, seed_len,
                   true, true);
...
void handshake_prf_circ_test() {
    ...
    tlsprf.opt_prf(hmac, ms, 384, pms, mk_label, mk_label_len, mk_seed,
                   mk_seed_len, true, true);

    Integer sk;
    tlsprf.init(hmac, ms);
    tlsprf.opt_prf(hmac, sk, 320, ms, ke_label, ke_label_len, ke_seed,
                   ke_seed_len, true, true);

    Integer ucfin;
    tlsprf.opt_prf(hmac, ucfin, 96, ms, cfin_label, cfin_label_len, ctau,
                   ctau_len, true, true);

    Integer usfin;
    tlsprf.opt_prf(hmac, usfin, 96, ms, sfin_label, sfin_label_len, stau,
                   stau_len, true, true);
