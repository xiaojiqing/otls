...
class OLE {
   public:
        ...
        if (!cmpBlock(&ot->Delta, &zero_block, 1)) {
            ...
            for (int i = 0; i < out.size(); ++i) {
                ...
                for (int j = 0; j < bit_length; ++j) {
                    H(pad1, raw[i * bit_length + j], q, ccrh);
                    H(pad2, raw[i * bit_length + j] ^ ot->Delta, q, ccrh);
            ...
            for (int i = 0; i < out.size(); ++i) {
                ...
                for (int j = 0; j < bit_length; ++j) {
                    ...
                    H(msg, raw[i * bit_length + j], q, ccrh);
    ...
    uint64_t H(block b, uint64_t q) { return b[0] % q; }
        ...
        if (!cmpBlock(&ot->Delta, &zero_block, 1)) {
            ...
            for (int i = 0; i < out.size(); ++i) {
                ...
                for (int j = 0; j < 64; ++j) {
                    ...
                    msg[i * 64 + j] =
                      (pad[i * 64 + j] + H(raw[i * 64 + j] ^ ot->Delta, q) + in[i]) % q;
